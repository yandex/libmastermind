...
lookup_result_t::lookup_result_t(const ioremap::elliptics::lookup_result_entry &entry, bool eblob_style_path, int base_port)
	: m_entry(entry)
...
const std::string &lookup_result_t::host() const
{
	if (!m_host) {
...
uint16_t lookup_result_t::port() const
{
	if (!m_port) {
		struct dnet_addr *addr = m_entry.storage_address();
...
int lookup_result_t::group() const
{
	return m_entry.command()->id.group_id;
}

int lookup_result_t::status() const
{
	return m_entry.command()->status;
}

const std::string &lookup_result_t::addr() const
{
	if (!m_addr) {
		char addr_dst[512];
		struct dnet_addr *addr = m_entry.storage_address();
...
const std::string &lookup_result_t::full_path() const
{
	if (!m_full_path) {
		struct dnet_file_info *info = m_entry.file_info();
